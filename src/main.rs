...
use crate::protocol::packet::{HelloPacket, Packet, PingPacket, ReliablePacket};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            unsafe {
                if CONNECTIONS.lock().unwrap().contains_key(&data_address) {
                    ...
                    CONNECTIONS.lock().unwrap().insert(data_address, Some(user));
                }
            }
        }
    });
